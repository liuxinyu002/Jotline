---
name: openspec-arch-audit
description: Run a four-phase architecture audit (context gathering, read-only deep-module walkthrough, HTML report, grilling loop) that surfaces shallow modules, seam leaks and refactor candidates, writes a dated HTML report, and bootstraps decision records (CONTEXT.md / ADR) without touching application code. Use when the user wants to audit or improve codebase architecture, find architectural friction, get refactor proposals, or start recording architecture decisions as ADRs.
license: MIT
